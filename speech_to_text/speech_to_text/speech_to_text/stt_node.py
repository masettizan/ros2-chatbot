...
class STTNode(Node):  # pylint: disable=too-many-instance-attributes
    ...
    #written to not consistently process messages
    ...
    def listen_from_mic(self) -> None:
        ...
        while self.started and rclpy.ok():
            ...
            with self.__mic as source:
                self.get_logger().info("listening to audio...")
                ...
                try:
                    if self.service == "sphinx":
                        ...
                        self.get_logger().info("recognizing audio...")
                    ...
                    self.get_logger().info('Published result: "%s"' % stt_result)
                    self.get_logger().info("Started flag at publishing: " + str(self.started))
                    break
            self.get_logger().info("End of listening loop. Started flag: " + str(self.started))
            if not self.started:
                break
